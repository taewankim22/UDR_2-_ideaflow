...
const notifications = [
  { icon: <MessageCircle size={16} />, title: "새 댓글", detail: "Mina님이 내 아이디어에 댓글을 남겼습니다." },
  { icon: <Heart size={16} />, title: "좋아요", detail: "Joon님이 아이디어를 좋아합니다." },
  { icon: <UserPlus size={16} />, title: "새 팔로우", detail: "새 사용자가 나를 팔로우했습니다." }
];
  ...
  return (
    <header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-slate-200 bg-white/90 px-5 backdrop-blur lg:px-8">
      ...
      <div className="ml-auto flex items-center gap-3 text-brand-700">
        <button
          className="grid h-10 w-10 place-items-center rounded-full hover:bg-brand-50"
          onClick={onRefresh}
          title="새로고침"
        >
        ...
        <div className="relative">
          <button
            className={`relative grid h-10 w-10 place-items-center rounded-full transition ${
              notificationOpen ? "bg-brand-600 text-white" : "hover:bg-brand-50"
            }`}
            onClick={() => {
            ...
            <div className="absolute right-0 mt-3 w-72 rounded-[22px] border border-slate-200 bg-white p-4 text-slate-950 shadow-soft">
              ...
              <div className="grid gap-2">
                {notifications.map((item) => (
                  <button
                    key={item.title}
                    className="flex gap-3 rounded-2xl bg-slate-50 p-3 text-left transition hover:bg-brand-50"
                  >
